...
export default function LoadingScreen() {
  const letters = "Morning Coffee".split("");

  return (
    <div className="fixed inset-0 bg-lightbrown flex flex-col items-center justify-center">
      <div className="w-64 h-64 relative">
        ...
        >
          ...
          {[0, 1, 2].map((i) => (
            <motion.path
              key={i}
              d={`M${800 + i * 200},400 Q${850 + i * 200},200 ${900 + i * 200},400`}
              fill="none"
              stroke="#472713"
              strokeWidth="40"
      ...
      <div className="flex mt-4">
        {letters.map((letter, index) => (
          <motion.div
            key={index}
            className={`text-6xl sm:text-6xl font-semibold text-coffee ${boong.className}`} 
            animate={{
              y: [0, -40, 0],
