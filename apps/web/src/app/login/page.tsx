...
import StartShift from './_components/StartShift';
import { useState } from 'react';
import ShiftModal from '../(home)/(cashier)/_components/ShiftModal';
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden">
      ...
      <div className="absolute right-10 h-screen z-20 flex flex-col top-10 space-y-20  items-center">
        <Image src={'/images/logo/logo-white.png'} width={180} height={200} alt="" />
        <div>
          <h2 className={`text-6xl font-semibold text-center text-white mb-6 ${boong.className}`}>Welcome back</h2>
          <LoginForm setOpenModal={setOpenModal} />
        </div>
      </div>
      {openModal && <ShiftModal children={<StartShift />} closeModal={() => setOpenModal(false)} />}
    </div>
  );
}
